...
export const addToHistory = async (
  ...
  try {
    ...
    // Validation spécifique pour searchName
    if (interactionType === 'searchName') {
      // Vérifier que c'est bien une recherche
      if (!productCode.startsWith('search_')) {
        productCode = 'search_' + new Date().getTime();
      }
      
      // S'assurer que les métadonnées sont correctes
      console.log("Métadonnées de recherche:", productMetadata);
    }
    ...
    console.log("Données envoyées à product_history:", {
      user_id: userId,
      ...
      search_criteria: productMetadata.search_criteria || null
    });
