...
const useProductSearch = () => {
  ...
  const [searchOrigin, setSearchOrigin] = useState('direct');
    // Ajout d'une référence pour éviter les boucles infinies
  ...
  useEffect(() => {
       // Ne pas exécuter si nous sommes en train de mettre à jour l'URL
  ...
  // Mise à jour des paramètres d'URL
   // Modifier updateUrlParams pour éviter les mises à jour en boucle
  const updateUrlParams = (type, value, shouldTriggerSearch = false) => {
    ...
    const currentValue = type === 'barcode' ? currentParams.get('barcode') : currentParams.get('q');
    
    // Ne pas mettre à jour si la valeur est identique
    if (value === currentValue) return;
    
    isUserInitiatedSearch.current = shouldTriggerSearch;
    isUpdatingUrl.current = true;  // Indiquer que nous mettons à jour l'URL
...
const handleNameSearch = async (newSearch = true, searchQuery = null, filters = null, isInitialLoad = false) => {
  ...
  // ===== DÉPLACER L'ENREGISTREMENT HISTORIQUE ICI =====
  // Enregistrer la recherche dans l'historique AVANT la recherche
  if (newSearch && !isInitialLoad && currentUser && userDetails && queryToSearch.trim()) {
    ...
    // Note: on met 0 ou null pour totalResults car on ne connaît pas encore le résultat
    const searchMetadata = {
      product_name: queryToSearch.trim(),
      search_criteria: searchCriteria,
      total_results: 0 // Valeur provisoire
    };
    
    try {
      console.log("Enregistrement de la recherche AVANT exécution:", {
        userId: userDetails.id,
        productCode: 'search_' + new Date().getTime(),
        type: 'searchName',
      ...
      const historyResult = await addToHistory(
        ...
        'searchName',
        searchMetadata,
        0 // Valeur provisoire pour totalResults
      );
      
      if (historyResult && historyResult.success) {
        console.log("Recherche ajoutée à l'historique avec succès");
        incrementUsage('searchName');
      } else {
        console.warn("Échec de l'ajout à l'historique:", historyResult?.error);
      }
    } catch (historyError) {
  ...
  try {
    ...
    // === RETIRER LE BLOC D'ENREGISTREMENT D'HISTORIQUE ICI ===
    // (puisqu'on l'a déplacé avant la recherche)
