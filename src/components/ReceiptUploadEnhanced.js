// src/components/ReceiptUploadEnhanced.js (version corrigée)
...
const ReceiptUploadEnhanced = ({ onUploadComplete, productCode = null, productName = null }) => {
  ...
  // Fonction pour redimensionner l'image si elle est trop lourde
  const resizeImage = useCallback((file, maxWidth = 1920, maxHeight = 1080, quality = 0.8) => {
    return new Promise((resolve) => {
      ...
      img.onerror = () => {
        console.error('Erreur lors du redimensionnement de l\'image');
  ...
  // Gestion de la sélection du fichier avec gestion d'erreurs améliorée
  const handleFileChange = useCallback(async (e) => {
    console.log('🔄 Début de handleFileChange');
    ...
    try {
      ...
      // Vérification de la taille du fichier (max 10 MB avant redimensionnement)
      ...
      // Redimensionner si l'image est trop lourde (> 2MB)
      if (selectedFile.size > 2 * 1024 * 1024) {
        console.log('📐 Redimensionnement de l\'image...');
        try {
          processedFile = await resizeImage(selectedFile);
          console.log('✅ Image redimensionnée:', {
            originalSize: selectedFile.size,
            newSize: processedFile.size
          });
        } catch (resizeError) {
          console.error('⚠️ Erreur lors du redimensionnement:', resizeError);
      ...
      // Création d'un aperçu avec gestion d'erreurs
      try {
        const reader = new FileReader();
        
        reader.onload = (event) => {
          console.log('✅ Aperçu créé avec succès');
        ...
        setError("Erreur lors de la préparation de l'aperçu");
      }
      
    } catch (globalError) {
      console.error('❌ Erreur globale dans handleFileChange:', globalError);
      setError("Erreur lors du traitement du fichier: " + globalError.message);
    } finally {
      // Nettoyer l'input file pour éviter les problèmes
      if (fileInputRef.current) {
        fileInputRef.current.value = '';
        // Supprimer l'attribut capture qui pourrait causer des problèmes
        fileInputRef.current.removeAttribute('capture');
      }
    }
  }, [currentUser, resizeImage]);
  ...
  const handleOpenFileSelector = useCallback(() => {
    if (fileInputRef.current) {
      // S'assurer que l'attribut capture n'est pas défini pour la galerie
      fileInputRef.current.removeAttribute('capture');
      fileInputRef.current.click();
    }
  }, []);
  
  // Ouverture directe de la caméra sur mobile - VERSION CORRIGÉE
  const handleOpenCamera = useCallback(() => {
    if (fileInputRef.current) {
      // Méthode plus sûre pour définir l'attribut capture
      try {
        fileInputRef.current.setAttribute('capture', 'environment');
        fileInputRef.current.click();
      } catch (error) {
        console.error('Erreur lors de l\'ouverture de la caméra:', error);
        // Fallback: ouvrir le sélecteur de fichier normal
        fileInputRef.current.removeAttribute('capture');
        fileInputRef.current.click();
      }
    }
      ...
      <div className="p-6">
        ...
        {!file ? (
          <div className="space-y-4">
            ...
            {/* Input file avec gestion d'erreurs améliorée */}
            <input
              type="file"
              accept="image/*"
              className="hidden"
