...
const SearchResultItem = ({ result, onSelect, searchFilters = {} }) => {
  ...
  const fetchAttempted = useRef(false); // Pour éviter les appels répétés
  ...
  useEffect(() => {
    // Si les données essentielles sont déjà présentes, ne pas charger
    ...
    // Si déjà chargé ou en cours de chargement, ne pas recharger
    ...
    const fetchProductDetails = async () => {
      setLoading(true);
      fetchAttempted.current = true; // Marquer comme tentative effectuée
      
      try {
        // Vérifier si le produit est déjà dans le cache local
        ...
        if (cachedProduct) {
          try {
            ...
            // Si erreur de parsing, ignorer le cache
          }
        }

        const { success, data } = await getProductDetails(result.code);
        if (success && data) {
          // Ajouter timestamp pour la gestion du cache
    ...
    fetchProductDetails();
  }, [result.code]); // Dépendance simplifiée pour éviter les appels répétés
