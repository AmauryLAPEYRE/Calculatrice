    ...
    for (const [key, similarProduct] of Object.entries(similarProducts)) {
      try {
        ...
        if (productCode === similarCode) {
          ...
          return { success: false, error: 'Auto-référence', inserted: 0 };
        }
        ...
        //const similarExistsPromise = ensureProductExists(supabase, similarCode);
        const similarExistsPromise = true;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const requiredParams = ['productCode', 'productName', 'userId'];
    ...
    // Initialisation Supabase avec test de connexion
...
- IMPORTANT: trouve 2-3 produits similaires avec leurs codes-barres réels et leur score de similarité
...
**Format de réponse JSON attendu :**
{
  ...
  "tasteRating": [note de 1.0 à 5.0],
  "quantityRating": [note de 1.0 à 5.0], 
  "priceRating": [note de 1.0 à 5.0],
...
- Utilise des codes-barres réels de 13 chiffres
- similarity_score entre 0.0 et 1.0 (1.0 = identique)
- similarity_reason: 'same_brand', 'same_category', 'same_manufacturer', 'name_similarity'
- Trouve des produits vraiment existants et similaires
...
Retourne uniquement le JSON, sans explication supplémentaire.`;
    ...
    try {
      openaiResponse = await openai.chat.completions.create({
        ...
        max_tokens: 1500, // Augmenté pour inclure les produits similaires
        temperature: 0.7
      });
    ...
    let similarProductsResult = { success: true, inserted: 0 };
    if (aiReview.similar_products) {
      similarProductsResult = await insertSimilarProducts(
        supabase, 
        params.productCode, 
        aiReview.similar_products
      );
    }
