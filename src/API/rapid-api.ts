...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    try {
      ...
      const promptText = `
		Analyser cette image et déterminer d'abord s'il s'agit d'un ticket de caisse.

		IMPORTANT: Avant toute analyse, vérifier si l'image montre bien un ticket de caisse/facture.

		Retourner un JSON avec la structure suivante:
		{
		"is_receipt": true/false,
		"confidence_score": [0-1], // Niveau de confiance dans la détection
		"detection_reason": "Explication courte de la décision",
        "receipt": {
          "enseigne": {
	  ...
	  Si l'image n'est PAS un ticket de caisse, retourner uniquement:
	{
	"is_receipt": false,
	"confidence_score": [0-1],
	"detection_reason": "Raison précise pour laquelle ce n'est pas un ticket"
	}
